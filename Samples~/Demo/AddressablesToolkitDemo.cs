...
namespace KidzDev.AddressablesToolkit.Samples
{
    ...
    ///
    /// Note how this depends on the toolkit's <see cref="IAssetLoader"/> / <see cref="IAssetPool"/>
    /// abstractions (resolved here to the process-wide defaults) rather than the static facades —
    /// the same seams you'd inject a test double into.
    /// </summary>
    public class AddressablesToolkitDemo : MonoBehaviour
    {
        ...
        // Depend on the abstractions; swap these for your own implementations (or fakes in a test).
        private readonly IAssetLoader _assetLoader = AssetLoader.Default;
        private readonly IAssetPool _assetPool = AddressablePool.Default;

        private async UniTaskVoid Start()
        {
            try
            {
                // 1) IAssetLoader — reference-counted load (cache keyed by address + type).
                var prefab = await _assetLoader.LoadAsync<GameObject>(prefabAddress);
                ...
                // 2) IAssetPool — pooled instances reuse the same prefab handle.
                await _assetPool.Prewarm(prefabAddress, 3);
                for (int i = 0; i < 3; i++)
                {
                    var pooled = await _assetPool.GetAsync(prefabAddress);
                    pooled.transform.position = new Vector3(i, 0f, 0f);
                }
                Debug.Log("[Demo] Spawned 3 instances from the pool.");

                // 3) ContentDownloader — bytes still to download (0 for local content).
                long size = await ContentDownloader.GetDownloadSizeAsync(prefabAddress);
                ...
                Destroy(single);
                _assetLoader.Release<GameObject>(prefabAddress);
            }
            catch (Exception e)
            {
        ...
        private void OnDestroy() => _assetPool.ClearPool(prefabAddress);
    }
}
